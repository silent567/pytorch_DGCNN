...
def build_edges(graph):
    edges = np.reshape(graph.edge_pairs,[-1,2])
    return edges

class MultiAttPool(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1]
        ...
        super(MultiAttPool, self).__init__()
        self.latent_dim = latent_dim
        self.output_dim = output_dim
        self.num_node_feats = num_node_feats
        self.num_edge_feats = num_edge_feats
        ...
        self.att_aggrs = [FlexAddAttention(self.dense_dim,dim_func(i),None,max_type,layer_norm_flag,lam,gamma) for i in range(head_cnt)]
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs, graph_list):
        ...
        '''Attentional pooling'''
        adjacency_matrices = [build_graph(graph_list[i])for i in range(len(graph_sizes))]
        to_dense = torch.cat([torch.cat([att_aggr(cur_message_layer[graph_size_cumsum[i]:graph_size_cumsum[i+1]].unsqueeze(0)
                            ,adjacency_matrices[i]) for i in range(len(graph_sizes))], dim = 0)
                             for att_aggr in self.att_aggrs], dim = -1)
...
class FastAttPool(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1]
        ...
        self.batch_norm = torch.nn.BatchNorm1d(self.total_latent_dim) if batch_norm_flag else lambda x:x
        self.att_aggr = FastFlexAddAttention(self.dense_dim,self.dense_dim,None,max_type,layer_norm_flag,lam,gamma)
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs, graph_list):
        ...
        '''Attentional pooling'''
        to_dense = self.att_aggr([cur_message_layer[graph_size_cumsum[i]:graph_size_cumsum[i+1]] for i in range(len(graph_sizes))]
                                  ,[build_edges(graph_list[i]) for i in range(len(graph_sizes))])
